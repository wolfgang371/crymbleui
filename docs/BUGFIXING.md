...
Six durable instruments replace the old ad-hoc SFML autotest pile (which violated the
...
| Clip containment | `tools/clip-containment-probe.cr` — build, then `DISPLAY=:0 ./bin/clip-containment-probe` (exit 0 GREEN, 1 RED, 2 no verdict) | That a clip pushed on a `CrSFMLBackend` governs every later draw on it until popped, INCLUDING the first draw after a render-target re-activation. Each interlude is a real `layer_renderer.cr` sequence (construct another backend, push/pop a clip on another backend, suspend/resume, `display()` mid-clip), plus nested-intersection, disjoint-clips-draw-nothing, cold `draw_text`, glyph-atlas growth, `clear()` under a clip, a SPRITE BLIT under a clip (case M — the widget-texture blit-to-layer step), the over-clipping controls (a fitting draw must be byte-identical to unclipped, tolerance 0) and the scissor factor round trip. No headless spec can see ANY of it — `TestRenderBackend` clips in software with no GL context — which is why a green suite coexisted with the field report. Blind spots: bare `RenderTexture` only (says nothing about the RenderWindow target or the compositor; the widget-texture blit-to-layer step IS now covered, by case M); one driver, one context, SFML-version dependent; exact colour compare, so a sub-pixel escape is invisible; and it is NOT run by CI (no display) — run it beside `tools/sfml-parity.sh`. The CI-visible half is tripwires (f), (g) and (h) in `spec/rendering/instrument_tripwires_spec.cr` |
...
| Multi-line text metrics | `tools/multiline-probe.cr` — build, then `./bin/multiline-probe` from the repo root (exit 0 GREEN, 1 RED, 2 no verdict) | The four production text facts headless cannot model, because `TestFont` has no glyphs and reports `reference_height == size`: that `measure_text(...).width` for a multi-line string is the WIDEST line; that its height is line_spacing x line_count with a single line byte-identical; that `reference_height` (a line's INK extent) genuinely DIFFERS from the line SLOT — headlessly they are equal, so block-geometry arithmetic that confuses the two is green in the suite and wrong on screen; and each line's own `local_bounds.left` versus the BLOCK's, which is the caret's drift from its glyphs when a block is drawn with one `draw_text` but the caret is placed from a per-line measurement. Blind spots: one font at three sizes, and metrics only — it says nothing about where ink actually lands |

The headless text model claims LINE STRUCTURE as of the multi-line work: `TestFont#measure_text`
reports the widest line and `TestFont.line_step(size) * line_count`, and `TestRenderBackend#draw_text`
resets x and advances y by that same slot per `\n`. The slot is published by the font and consumed by
the backend precisely so the two cannot disagree about where line k sits — a divergence there is
invisible to every example that uses both. What it does NOT model: absolute line spacing (no real
font, so the slot is the em size rather than SFML's `get_line_spacing`) and the ink-versus-slot
difference (`reference_height` defaults to the em size headlessly). Both are why the probe above
exists; a block-geometry claim that rests on either must be measured there, not asserted in a spec.
