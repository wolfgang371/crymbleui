...
module CrymbleUI
  ...
  class ComboBox < Widget
    ...
    # the collapsed cell can display it; reconcile_property so it survives a DSL
    # rebuild like selected_index does. nil for a normal item pick.
    reconcile_property custom_text : String? = nil
    ...
    def select_and_close(index : Int32, value : String)
      ...
      @custom_text = item_index?(index) ? nil : value
    ...
    def copy_state_from(old_widget : Widget)
      ...
      if @popup_open && (popup = old.@current_popup)
        ...
        # (the arrow intercepts are filter-mode-gated and not preserved here — see T-015).
        wire_popup_tab_callback(popup)
      end
    end
    ...
    # and collapse returns focus to it. Returns nil for a standalone ComboBox.
